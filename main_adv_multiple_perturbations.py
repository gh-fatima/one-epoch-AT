...
net.cuda()
PATH = '/home/Fatemeh/One-epoch/EMP-SSL-main/logs/EMP-SSL-Training/patchsim200_numpatch40_bs100_lr0.3_NONE/save_models_adv_wo_Normalization_8_patch_multiple_perturbation/29.pt'
...
def pgd_linf(model, X, epsilon, alpha, num_iter,criterion):    
    ...
    for epoch in range(args.epoch):            
        for step, (data, label) in tqdm(enumerate(dataloader)):
            ...
            if (epoch%3==0):
                ...
                alpha = (16/255)
            delta = pgd_linf(net, data, alpha, 1e-2, 5, criterion)#64/255 for CIFAR10 #8/255 for CIFAR100
            X_adv = (data + delta)
            #delta2 = pgd_linf(CLNet, x0, 16/255, 1e-2, 5, criterion_adv,labels,method) #2/255 for CIFAR10 #16/255 for CIFAR100
            #X_adv2 = (x0 + delta2)
          # Forward pass
            z_proj = net(X_adv)
            z_list = z_proj.chunk(num_patches, dim=0)
        ...
        model_dir = dir_name+"/save_models_adv_wo_Normalization_8_patch_multiple_perturbation/"
        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        torch.save(net.state_dict(), model_dir+str(epoch+30)+".pt")
        
    
        print("At epoch:", epoch+30, "loss similarity is", loss_contract.item(), ",loss TCR is:", (loss_TCR).item(), "and learning rate is:", opt.param_groups[0]['lr'])
