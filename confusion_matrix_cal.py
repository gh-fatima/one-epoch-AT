...
def plot_confusion_matrix(cm, class_labels, file_name, figsize=(12, 12)):
    ...
    # Save the confusion matrix plot as a figure
    plt.savefig("./figures/"+file_name)  # Specify the desired file name and format here

def testEvalNet_adv(eps):
    net.eval()
    LL.eval()
    total_acc_test = 0
    true_labels = []  # Initialize a list to store true labels
    pred_labels = []  # Initialize a list to store predicted labels
    ...
    for i, (X, labels) in tqdm(enumerate(testLoader)):
        ...
        pred_labels.extend(yp.max(dim=1)[1].cpu().numpy())
    ...
    # Plot and save the confusion matrix with cell values
    class_labels = [str(i) for i in range(args.num_class)]  # Replace with your class labels
    plot_confusion_matrix(conf_matrix, class_labels, 'confusion_matrix_adv_lecun.png')

    return total_acc_test / len(testLoader.dataset)

accuracy = testEvalNet_adv(8/255)
...
def testEvalNet_adv_Ro_end(epst):
    ...
    # Plot and save the confusion matrix with cell values
    class_labels = [str(i) for i in range(args.num_class)]  # Replace with your class labels
    plot_confusion_matrix(conf_matrix, class_labels, 'confusion_matrix_adv_normal.png')

    return total_acc_test / len(testLoader.dataset)

accuracy = testEvalNet_adv_Ro_end(8/255)
